...
import br.com.countbellydash.dao.Conexao;
import br.com.countbellydash.model.Client;
...
public class DashboardService {
    ...
    private static final String API_URL = dotenv.get("SESSIONS_API_URL");
    ...
    public List<AgendaItem> getFutureAgenda(List<AgendaItem> fullAgenda) {
        LocalDateTime now = LocalDateTime.now();
        return fullAgenda.stream()
            .filter(item -> item.status.equals("Agendada") && LocalDateTime.parse(item.dataHora, DATE_FORMATTER).isAfter(now))
            .sorted(Comparator.comparing(item -> LocalDateTime.parse(item.dataHora, DATE_FORMATTER)))
            .limit(5) 
            .collect(Collectors.toList());
    }

    public List<AgendaItem> getPastAgenda(List<AgendaItem> fullAgenda) {
        LocalDateTime now = LocalDateTime.now();
        return fullAgenda.stream()
            .filter(item -> !item.status.equals("Agendada") || (item.status.equals("Agendada") && !LocalDateTime.parse(item.dataHora, DATE_FORMATTER).isAfter(now))) 
            .sorted(Comparator.comparing((DashboardService.AgendaItem item) -> LocalDateTime.parse(item.dataHora, DATE_FORMATTER)).reversed())
            .limit(5) 
            .collect(Collectors.toList());
    }

    public List<Client> fetchClientsFromSessionAPI() {
        if (API_URL == null || API_URL.isEmpty()) {
            System.err.println("SESSIONS_API_URL não configurada no arquivo .env");
            return Collections.emptyList();
        }

        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(API_URL +"/sessions"))
                    .GET()
                    .build();
            ...
            if (response.statusCode() == 200) {
                return objectMapper.readValue(response.body(), new TypeReference<List<Client>>() {});
            } else {
                System.err.println("Erro ao buscar clientes da API de sessões: Status " + response.statusCode());
